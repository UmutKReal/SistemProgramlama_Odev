        ...
        show_menu();
            ...
            switch (choice) {
                ...
                case 3:
                    ...
                    size = strlen(data);
                    int bytes_written = fs_write(filename, data, size);

                    if (bytes_written == size)
                        printf("Yazma başarılı (%d byte).\n", bytes_written);
                    else
                        printf("Hata: Yazılamadı.\n");
                    break;

                case 4://dosyadan veri oku
                    printf("Dosya adı: ");
                    fgets(filename, MAX_FILENAME, stdin); strtok(filename, "\n");
                    printf("Offset (başlangıç pozisyonu): "); 
                    scanf("%d", &offset);
                    printf("Boyut (kaç byte okunacak): "); 
                    scanf("%d", &size); 
                    getchar();
                    
                    printf("[DEBUG] Dosya okuma isteği: %s, offset=%d, size=%d\n", filename, offset, size);
                    ...
                    // Buffer'ı belirli bir değerle doldur (karakter kontrolü için)
                    memset(buffer, 'X', size);
                    buffer[size] = '\0';
                    
                    int read_result = fs_read(filename, offset, size, buffer);
                    if (read_result == 0) {
                        buffer[size] = '\0'; // NULL terminatör eklendiğinden emin ol
                        
                        // Buffer içeriğini hem normal string olarak hem de hex olarak göster
                        printf("Okunan veri: \"%s\"\n", buffer);
                        
                        printf("Hex olarak buffer içeriği: ");
                        for (int i = 0; i < size; i++) {
                            printf("%02X ", (unsigned char)buffer[i]);
                        }
                        printf("\n");
                    } else {
                        printf("Hata: Okuma başarısız oldu (kod: %d).\n", read_result);
                    }
